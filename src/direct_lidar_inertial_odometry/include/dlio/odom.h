...
private:
  ...
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr deskewed_organized_pub;  
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr imu_cg_pub_;
  // NEW: dense trajectory path
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr dense_path_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr dense_pose_pub_;
  ...
  // NEW: dense trajectory path buffer
  nav_msgs::msg::Path dense_path_ros_;
  // Organized deskewed output options
  bool publish_deskewed_organized_;
  int pano_width_;
  int pano_height_;
  int col_offset_;  
  // Flags
  ...
  std::mutex keyframes_mutex;
	std::vector<double> dense_timestamps_;
	std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> dense_frames_;
  // Sensor Type
  ...
  bool dense_traj_enable_;
  std::string dense_traj_topic_;
