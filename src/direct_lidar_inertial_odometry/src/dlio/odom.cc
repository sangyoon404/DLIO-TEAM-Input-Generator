...
namespace
{
  inline std::uint16_t azimuth_to_col(float x, float y, int W, int offset)
  {
    constexpr float PI = 3.14159265358979323846f;
    float a = std::atan2(y, x);       // -pi .. pi
    float t = (a + PI) / (2.0f * PI); // 0 .. 1
    int col = static_cast<int>(t * static_cast<float>(W));
    col += offset;
    col %= W;
    if (col < 0)
      col += W;
    if (col >= W)
      col = W - 1;
    return static_cast<std::uint16_t>(col);
  }
}
...
dlio::OdomNode::OdomNode() : Node("dlio_odom_node")
{
  ...
  this->imu_cg_pub_ = this->create_publisher<sensor_msgs::msg::Imu>(
      "imu_cg", rclcpp::SensorDataQoS());
  ...
  this->deskewed_organized_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("deskewed_organized", 1);
  ...
  // after getParams();
  // if (this->dense_traj_enable_) {
  //   this->dense_pose_pub_ =
  //       this->create_publisher<geometry_msgs::msg::PoseStamped>("dense_pose", 100);
  // }
  /*
  // NEW: dense trajectory path publisher
  if (this->dense_traj_enable_) {
    this->dense_path_pub =
      this->create_publisher<nav_msgs::msg::Path>(this->dense_traj_topic_, 1);
    this->dense_path_ros.header.frame_id = this->odom_frame;
  }
  */
...
void dlio::OdomNode::getParams()
{
  ...
  // Organized deskewed output for 2D mask mapping
  dlio::declare_param(this, "pointcloud/deskewed_organized/enable", this->publish_deskewed_organized_, false);
  dlio::declare_param(this, "pointcloud/deskewed_organized/pano_width", this->pano_width_, 1024);
  dlio::declare_param(this, "pointcloud/deskewed_organized/pano_height", this->pano_height_, 0);
  dlio::declare_param(this, "pointcloud/deskewed_organized/col_offset", this->col_offset_, 0);
  ...
  // Dense trajectory options
  dlio::declare_param(this, "odom/dense_traj/enable", this->dense_traj_enable_, false);
  dlio::declare_param(this, "odom/dense_traj/topic", this->dense_traj_topic_, std::string("dense_path"));
  ...
  // --- 새 range 필터 파라미터 ---
  dlio::declare_param(this, "odom/preprocessing/range/enable", this->range_filter_enable_, false);
  dlio::declare_param(this, "odom/preprocessing/range/min", this->range_min_, 0.0);
  dlio::declare_param(this, "odom/preprocessing/range/max", this->range_max_, 100.0);
  // -----------------------------
  ...
  // NEW: dense trajectory export
  if (this->dense_traj_enable_)
  {
    // Per-pose publisher (for logging to bag, TUM conversion, etc.)
    this->dense_pose_pub_ =
        this->create_publisher<geometry_msgs::msg::PoseStamped>("dense_pose", 100);

    // Path publisher (for RViz visualization, etc.)
    // this->dense_path_pub_ =
    //     this->create_publisher<nav_msgs::msg::Path>(this->dense_traj_topic_, 1);

    // this->dense_path_ros_.header.frame_id = this->odom_frame;
  }
}
...
void dlio::OdomNode::publishCloud(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud)
{
  ...
  // Optional organized deskewed cloud for 2D mask mapping (ring x column)
  if (this->publish_deskewed_organized_)
  {
    const int W = this->pano_width_;
    int H = this->pano_height_;
    if (H <= 0)
    {
      std::uint16_t max_ring = 0;
      for (const auto &pt : deskewed_scan_t_->points)
      {
        if (pt.ring > max_ring)
          max_ring = pt.ring;
      }
      H = static_cast<int>(max_ring) + 1;
    }
    if (H > 0 && W > 0)
    {
      pcl::PointCloud<PointType> org;
      org.width = static_cast<std::uint32_t>(W);
      org.height = static_cast<std::uint32_t>(H);
      org.is_dense = false;
      org.points.resize(static_cast<std::size_t>(W) * static_cast<std::size_t>(H));
      ...
      for (int r = 0; r < H; ++r)
      {
        for (int c = 0; c < W; ++c)
        {
          auto &p = org.points[static_cast<std::size_t>(r) * W + c];
          p.x = NaN;
          p.y = NaN;
          p.z = NaN;
          p.intensity = 0.f;
          p.ring = static_cast<std::uint16_t>(r);
          p.column = static_cast<std::uint16_t>(c);
          p.t = 0;
        }
      }
      ...
      for (const auto &pt : deskewed_scan_t_->points)
      {
        if (!std::isfinite(pt.x) || !std::isfinite(pt.y) || !std::isfinite(pt.z))
          continue;
        if (pt.ring >= static_cast<std::uint16_t>(H))
          continue;
        if (pt.column >= static_cast<std::uint16_t>(W))
          continue;
        org.points[static_cast<std::size_t>(pt.ring) * W + pt.column] = pt;
      }

      sensor_msgs::msg::PointCloud2 org_ros;
      pcl::toROSMsg(org, org_ros);
      org_ros.header.stamp = this->scan_header_stamp;
      org_ros.header.frame_id = this->odom_frame;

      // auto key = scan_header_stamp.nanoseconds();
      // RCLCPP_INFO(this->get_logger(), "deskewed_org key %ld", static_cast<long>(key));

      this->deskewed_organized_pub->publish(org_ros);
    }
  }
}
...
void dlio::OdomNode::getScanFromROS(const sensor_msgs::msg::PointCloud2::SharedPtr &pc)
{
  ...
  // Preserve per-point ring and compute column for 2D mask mapping
  const int msgW = static_cast<int>(pc->width);
  const int msgH = static_cast<int>(pc->height);
  if (msgW == this->pano_width_ && msgH > 1 &&
      static_cast<int>(original_scan_->points.size()) == msgW * msgH)
  {
    for (std::size_t idx = 0; idx < original_scan_->points.size(); ++idx)
    {
      auto &pt = original_scan_->points[idx];
      pt.column = static_cast<std::uint16_t>(idx % msgW);
      pt.ring = static_cast<std::uint16_t>(idx / msgW);
    }
  }
  else
  {
    for (auto &pt : original_scan_->points)
    {
      ...
      {
        pt.column = azimuth_to_col(pt.x, pt.y, this->pano_width_, this->col_offset_);
      }
    }
  }
  ...
  // --- 새 range 필터 (센서 좌표계 기준) ---
  if (this->range_filter_enable_)
  {
    pcl::PointCloud<PointType>::Ptr range_filtered =
        std::make_shared<pcl::PointCloud<PointType>>();
    range_filtered->reserve(original_scan_->points.size());

    const float r_min = this->range_min_;
    const float r_max = this->range_max_;

    for (const auto &pt : original_scan_->points)
    {
      // 3D 거리 기준 (원하면 x,y만 쓰는 2D range로 바꿔도 됨)
    ...
    range_filtered->is_dense = original_scan_->is_dense;

    original_scan_ = range_filtered;
  }
...
void dlio::OdomNode::preprocessPoints()
{
  ...
  else
  {
    ...
    this->deskew_status = false;
    this->dense_frames_.clear();
    this->dense_timestamps_.clear();
  }
...
void dlio::OdomNode::deskewPointcloud()
{
  ...
  /*
  // NEW: export dense trajectory (baselink in odom/world) using frames + timestamps
  if (this->dense_traj_enable_ && this->dense_path_pub) {

    // 첫 호출 시 frame_id 설정
    if (this->dense_path_ros.header.frame_id.empty()) {
      this->dense_path_ros.header.frame_id = this->odom_frame;
    }
    ...
    for (std::size_t i = 0; i < frames.size(); ++i) {
      ...
      // timestamp : double [sec] -> rclcpp::Time [ns]
      const double t_sec = timestamps[i];
      ps.header.stamp = rclcpp::Time(static_cast<int64_t>(t_sec * 1e9));
      ps.header.frame_id = this->odom_frame;

      const Eigen::Matrix4f &T = frames[i];
      // baselink position in world/odom
      ps.pose.position.x = T(0, 3);
      ps.pose.position.y = T(1, 3);
      ps.pose.position.z = T(2, 3);

      // baselink orientation in world/odom
      ...
      this->dense_path_ros.poses.push_back(ps);
    }

    // Path 헤더 타임은 마지막 포인트 기준으로
    this->dense_path_ros.header.stamp =
      this->dense_path_ros.poses.back().header.stamp;

    this->dense_path_pub->publish(this->dense_path_ros);
  }
  */
  ...
  // IMU 기반 per-timestamp pose와 timestamp를 저장
  this->dense_frames_ = frames;
  this->dense_timestamps_ = timestamps;
...
void dlio::OdomNode::callbackImu(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
{
  ...
  // ★ 여기서 바로 publish 해서 시각화/로깅에 사용
  if (this->imu_cg_pub_)
  {
    this->imu_cg_pub_->publish(*imu);
  }
...
void dlio::OdomNode::getNextPose()
{
  ...
  if (this->dense_traj_enable_ && this->deskew_status && this->dense_pose_pub_)
  {

    // Only publish a dense stream of PoseStamped messages on /dense_pose.
    // We do NOT accumulate a nav_msgs::Path in memory.
    for (std::size_t i = 0; i < this->dense_frames_.size(); ++i)
    {
      Eigen::Matrix4f T_i = this->T_corr * this->dense_frames_[i];

      Eigen::Vector3f p_i = T_i.block<3, 1>(0, 3);
      Eigen::Matrix3f R_i = T_i.block<3, 3>(0, 0);
      Eigen::Quaternionf q_i(R_i);
      q_i.normalize();

      geometry_msgs::msg::PoseStamped pose;
      pose.header.frame_id = this->odom_frame;
      pose.header.stamp =
          rclcpp::Time(static_cast<int64_t>(this->dense_timestamps_[i] * 1e9));

      pose.pose.position.x = p_i.x();
      pose.pose.position.y = p_i.y();
      pose.pose.position.z = p_i.z();

      pose.pose.orientation.w = q_i.w();
      pose.pose.orientation.x = q_i.x();
      pose.pose.orientation.y = q_i.y();
      pose.pose.orientation.z = q_i.z();

      this->dense_pose_pub_->publish(pose);
    }
  }

  // Per-scan dense-trajectory buffers are always cleared
  this->dense_frames_.clear();
  this->dense_timestamps_.clear();
}
...
sensor_msgs::msg::Imu::SharedPtr dlio::OdomNode::transformImu(const sensor_msgs::msg::Imu::SharedPtr &imu_raw)
{
  ...
  imu->header.frame_id = this->baselink_frame;
  double imu_stamp_secs = rclcpp::Time(imu->header.stamp).seconds();
...
void dlio::OdomNode::computeSpaciousness()
{
  ...
  for (std::size_t i = 0; i < this->original_scan->points.size(); ++i)
  {
    float d = std::sqrt(
        this->original_scan->points[i].x * this->original_scan->points[i].x +
